...
void reset_segments() //function for reset the ssd segments
{
...
void reset_digits(){//function to reset ssd digits
	GPIOA->ODR |= (1U << 11);	//set  a5
	GPIOA->ODR |= (1U << 12);	//set  a4
	GPIOA->ODR |= (1U << 5);	//set  a3
	GPIOB->ODR |= (1U << 9);	//set  d10
...
void init_I2C(){
	...
	GPIOA->AFR[1] &= ~(0xFU << 1*4);
	GPIOA->AFR[1] |= (6 << 1*4);

	//PA10 alternate function mode
	GPIOA->AFR[1] &= ~(0xFU << 2*4);
	GPIOA->AFR[1] |= (6 << 2*4);

	//PA9 and PA10 open-drain mode on
	GPIOA->OTYPER |= (1U << 9);
	GPIOA->OTYPER |= (1U << 10);
	...
	I2C1 ->CR1 = 0; //set control register as zero
	I2C1 ->CR1 |= (1U << 7); // error interrupt ERRIE

	I2C1->CR2 |= (1U << 25);  // AOUTOEND enable
	I2C1->CR2 |= (1U << 14); //NACK enable
